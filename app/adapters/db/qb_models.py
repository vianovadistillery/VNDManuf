...
class FormulaClass(Base):
    ...
    __table_args__ = (
        Index("ix_formula_class_name", "name"),
    )
...
class ConditionType(Base):
    ...
    __table_args__ = (
        Index("ix_condition_type_code", "code"),
    )
...
class Dataset(Base):
    ...
    id = uuid_column()
    code = Column(String(3), unique=True, nullable=False, index=True)  # Default, BRI, HEN, etc.
