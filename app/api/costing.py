...
class RevaluationRequest(BaseModel):
    ...
    propagate: bool = Field(True, description="Whether to propagate to downstream assemblies")
...
async def inspect_cogs(
    item_id: str,
    as_of_date: Optional[datetime] = Query(None, description="Point-in-time date for historical costing"),
    include_estimates: bool = Query(True, description="Whether to include items with estimated costs"),
...
@router.post("/revalue", response_model=RevaluationResponse, status_code=status.HTTP_201_CREATED)
async def revalue_lot(
...
async def get_historical_cost(
    item_id: str,
    as_of_date: datetime = Query(..., description="Point-in-time date for historical costing"),
    db: Session = Depends(get_db)
):
    """
