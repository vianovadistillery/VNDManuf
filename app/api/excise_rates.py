...
async def create_excise_rate(data: ExciseRateCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        rate = ExciseRate(
            id=str(uuid4()),  # Explicitly set ID for SQLite compatibility
        ...
        logger.error(f"Database error creating excise rate: {str(e)}", exc_info=True)
        raise HTTPException(
        ...
        logger = logging.getLogger(__name__)
        logger.error(f"Unexpected error creating excise rate: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to create excise rate: {str(e)}",
        )
