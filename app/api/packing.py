...
from app.adapters.db.models import Product, PackUnit, PackConversion
from app.api.dto import PackConversionRequest, PackConversionResponse, ErrorResponse
from app.domain.rules import to_kg, to_liters, round_quantity
...
async def convert_pack_units(
    ...
    from_unit_obj = db.execute(select(PackUnit).where(PackUnit.code == from_unit)).scalar_one_or_none()
    to_unit_obj = db.execute(select(PackUnit).where(PackUnit.code == to_unit)).scalar_one_or_none()
    ...
    try:
        if from_unit.upper() in ["KG", "KILOGRAM"] and to_unit.upper() in ["L", "LITRE", "LITER"]:
        ...
        elif from_unit.upper() in ["L", "LITRE", "LITER"] and to_unit.upper() in ["KG", "KILOGRAM"]:
            if not product.density_kg_per_l:
                raise HTTPException(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    detail=f"Product {product.sku} has no density defined for kg conversion"
                )
            converted_qty = round_quantity(to_kg(qty, from_unit, product.density_kg_per_l).quantity_kg)
