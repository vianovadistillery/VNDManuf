...
from app.adapters.db.models import (
    Batch, BatchComponent, Formula, FormulaLine, 
    InventoryLot, InventoryTxn, Product
)
...
async def formula_cost_analysis(
    ...
    total_cost = sum(float(l.line_cost or 0) for l in lines)
        ...
        "lines": [
            {
                ...
                "material_desc": l.desc1,
                "quantity_kg": float(l.quantity_kg),
                "unit_cost": float(l.usage_cost or 0),
                "line_cost": float(l.line_cost or 0)
            }
            for l in lines
        ],
    ...
    return [
        {
            ...
            "variance_percent": float(b.variance_percent) if b.variance_percent else None,
            "status": b.status,
