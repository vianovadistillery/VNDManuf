...
def to_kg(quantity: Decimal, uom: str, density_kg_per_l: Optional[Decimal] = None) -> ConversionResult:
    ...
    raise ValueError(f"Unsupported unit conversion from {uom} to kg. Supported units: {list(MASS_TO_KG.keys()) + list(VOLUME_TO_L.keys())}")


def to_liters(quantity_kg: Decimal, density_kg_per_l: Decimal, target_unit: str = "L") -> Decimal:
    """
...
def convert_units(
    ...
    if from_unit_upper in MASS_TO_KG and to_unit_upper in VOLUME_TO_L:
        if density_kg_per_l is None:
            raise ValueError(f"Density required to convert from mass ({from_unit}) to volume ({to_unit})")
        ...
        factor = (MASS_TO_KG[from_unit_upper] / density_kg_per_l) / VOLUME_TO_L[to_unit_upper]
        return UnitConversionResult(round_quantity(converted), factor, from_unit, to_unit)
    
    # Volume to mass (requires density)
    if from_unit_upper in VOLUME_TO_L and to_unit_upper in MASS_TO_KG:
        if density_kg_per_l is None:
            raise ValueError(f"Density required to convert from volume ({from_unit}) to mass ({to_unit})")
        ...
        factor = (VOLUME_TO_L[from_unit_upper] * density_kg_per_l) / MASS_TO_KG[to_unit_upper]
        return UnitConversionResult(round_quantity(converted), factor, from_unit, to_unit)
...
def calculate_alcohol_quantity(
    ...
    if quantity_unit_upper in VOLUME_TO_L:
        ...
        if solution_density_kg_per_l is None:
            raise ValueError(f"Density required when converting from mass ({quantity_unit}) with ABV")
        ...
        raise ValueError(f"Unsupported quantity unit for alcohol calculation: {quantity_unit}")
    ...
    if target_unit_upper in MASS_TO_KG:
        ...
        factor = Decimal("1") / (alcohol_density_kg_per_l * VOLUME_TO_L[target_unit_upper])
    else:
...
def convert_concentration(
    ...
    if from_type_upper == "ABV_VOL_VOL" and to_type_upper == "WT_PCT":
        if solution_density_kg_per_l is None:
            raise ValueError("Solution density required to convert ABV to weight percentage")
        # ABV is vol/vol, convert to weight/weight
        # Alcohol density ~0.789 kg/L
        alcohol_density_kg_per_l = Decimal("0.789")
        abv_decimal = value / Decimal("100")
        wt_pct = (abv_decimal * alcohol_density_kg_per_l) / solution_density_kg_per_l * Decimal("100")
    ...
    if from_type_upper == "WT_PCT" and to_type_upper == "ABV_VOL_VOL":
        if solution_density_kg_per_l is None:
            raise ValueError("Solution density required to convert weight percentage to ABV")
        alcohol_density_kg_per_l = Decimal("0.789")
        wt_decimal = value / Decimal("100")
        abv_pct = (wt_decimal * solution_density_kg_per_l) / alcohol_density_kg_per_l * Decimal("100")
