...
class ConflictErrorResponse(JSONResponse):
    ...
    def __init__(
        ...
        content = {"error": "Conflict", "message": message, "request_id": request_id}
        if details:
...
class NotFoundErrorResponse(JSONResponse):
    ...
    def __init__(
        ...
        content = {"error": "Not Found", "message": message, "request_id": request_id}
...
def register_error_handlers(app: FastAPI) -> None:
    ...
    async def sqlalchemy_error_handler(request: Request, exc: SQLAlchemyError):
        ...
        log_request(
            request, f"SQLAlchemy error: {str(exc)}", level="error", sql_error=str(exc)
        )

        return InternalServerErrorResponse(
            "A database error occurred", request_id, include_details=False
        )
    ...
    async def http_exception_handler(request: Request, exc: HTTPException):
        ...
        if exc.status_code == 404:
            return NotFoundErrorResponse(str(exc.detail), request_id=request_id)
        elif exc.status_code == 409:
            return ConflictErrorResponse(str(exc.detail), request_id=request_id)
    ...
    async def general_exception_handler(request: Request, exc: Exception):
        ...
        return InternalServerErrorResponse(
            "An unexpected error occurred", request_id, include_details=False
        )
...
class InsufficientInventoryError(BusinessRuleViolation):
    ...
    def __init__(self, product_id: str, required_qty: float, available_qty: float):
        ...
        super().__init__(
            message, rule="inventory_check", entity="product", entity_id=product_id
        )
...
class InvalidUnitConversionError(BusinessRuleViolation):
    ...
    def __init__(self, from_unit: str, to_unit: str, product_id: Optional[str] = None):
        ...
        super().__init__(
            message, rule="unit_conversion", entity="product", entity_id=product_id
        )
...
class PricingResolutionError(BusinessRuleViolation):
    ...
    def __init__(
        self, product_id: str, pack_unit_id: str, customer_id: Optional[str] = None
    ):
        self.product_id = product_id
        ...
        super().__init__(
            message, rule="pricing_resolution", entity="product", entity_id=product_id
        )
