...
from app.adapters.db.models import (
    Product, InventoryLot, InventoryTxn, Revaluation, AssemblyCostDependency
)
from app.adapters.db.models_assemblies_shopify import Assembly as AssemblyModel
from app.domain.rules import (
    resolve_cost_for_product, resolve_cost_for_lot, CostResolution, CostSource,
    get_item_cost, round_money, round_quantity
)
...
class CostingService:
    ...
    def build_bom_tree(
        ...
        for assembly in assemblies:
            ...
            if child_tree['has_estimate']:
                ...
                if child_tree.get('estimate_reason'):
                    estimate_sources.append(f"{child_tree['sku']}: {child_tree['estimate_reason']}")
        ...
        if has_any_estimate:
            ...
            estimate_reason = "Contains estimated components: " + "; ".join(estimate_sources) if estimate_sources else "Contains estimated components"
        elif all(child['cost_source'] == CostSource.FIFO_ACTUAL.value for child in children):
    ...
    def inspect_cogs(
        ...
        # Get current or point-in-time cost
        if as_of_date:
    ...
    def revalue_lot(
        ...
        old_cost = lot.current_unit_cost or lot.original_unit_cost or lot.unit_cost or Decimal("0")
        ...
        if propagate:
            ...
            dependencies = self.db.query(AssemblyCostDependency).filter(
                AssemblyCostDependency.consumed_lot_id == lot_id
            ).all()
            ...
            for dep in dependencies:
                ...
                if produced_lot:
                    ...
                    consumed_issues = self._get_consumed_issues_for_produced_lot(produced_lot.id)
                    ...
                    new_parent_unit_cost = new_parent_cost / produced_lot.quantity_kg if produced_lot.quantity_kg > 0 else Decimal("0")
                    
                    # Update produced lot
                    old_parent_cost = produced_lot.current_unit_cost or produced_lot.unit_cost or Decimal("0")
                    ...
                    reval_parent = Revaluation(
                        ...
                        delta_extended_cost=(new_parent_unit_cost - old_parent_cost) * produced_lot.quantity_kg,
    ...
    def _get_consumed_issues_for_produced_lot(self, produced_lot_id: str) -> List[Dict[str, Any]]:
        """
        ...
        """
        dependencies = self.db.query(AssemblyCostDependency).filter(
            AssemblyCostDependency.produced_lot_id == produced_lot_id
        ).all()
        ...
        for dep in dependencies:
            ...
            if consumed_txn:
                ...
                current_cost = consumed_lot.current_unit_cost if consumed_lot else (consumed_txn.unit_cost or Decimal("0"))
                
                issues.append({
                    'lot_id': consumed_txn.lot_id,
                    'qty': abs(consumed_txn.quantity_kg),  # Abs because issues are negative
