    ...
    def convert(
        ...
        if from_unit.upper() in ["KG", "KILOGRAM"] and to_unit.upper() in ["L", "LITRE", "LITER"]:
            if not product.density_kg_per_l:
                raise ValueError(f"Product {product.sku} has no density defined for L conversion")
            ...
            if not product.density_kg_per_l:
                raise ValueError(f"Product {product.sku} has no density defined for kg conversion")
