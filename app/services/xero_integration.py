...
from app.services.xero_oauth import ensure_fresh_token, get_tenant_id
from app.adapters.db import get_session
from app.adapters.db.models import XeroSyncLog, Supplier, Customer, Product


API_BASE = "https://api.xero.com/api.xro/2.0"


def _log_sync(object_type: str, object_id: str, direction: str, status: str, message: str):
    ...
    with get_session() as session:
        ...
        payload = {
            "Contacts": [{
                ...
                "ContactPersons": [{"FirstName": supplier.contact_person or supplier.name}] if supplier.contact_person or supplier.name else [],
                "Phones": [{"PhoneNumber": supplier.phone}] if supplier.phone else [],
                "Addresses": [{"AddressLine1": supplier.address or ""}] if supplier.address else [],
                "IsSupplier": True,
                "IsCustomer": False,
            }]
        }
        ...
        try:
            ...
            _log_sync("Contact", supplier_id, "PUSH", "OK", f"Supplier synced to Xero")
    ...
    with get_session() as session:
        ...
        payload = {
            "Contacts": [{
                ...
                "ContactPersons": [{"FirstName": customer.contact_person or customer.name}] if customer.contact_person or customer.name else [],
                "Phones": [{"PhoneNumber": customer.phone}] if customer.phone else [],
                "Addresses": [{"AddressLine1": customer.address or ""}] if customer.address else [],
                "IsSupplier": False,
                "IsCustomer": True,
            }]
        }
        ...
        try:
            ...
            _log_sync("Contact", customer_id, "PUSH", "OK", f"Customer synced to Xero")
...
def push_product_as_item(product_id: str) -> Dict[str, Any]:
    ...
    with get_session() as session:
        ...
        try:
            ...
            _log_sync("Item", product_id, "PUSH", "OK", f"Product synced to Xero")
...
def post_batch_journal(batch_code: str, debit_account: str, credit_account: str, amount: Decimal, date_str: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        with get_session() as session:
            for contact in contacts:
                ...
                if is_supplier:
                    # Upsert as supplier
                    supplier = session.query(Supplier).filter_by(xero_contact_id=contact_id).first()
                    if not supplier:
                        supplier = Supplier(
                            code=contact.get("ContactNumber") or f"XERO-{contact_id[:8]}",
                            name=name,
                            email=email,
                ...
                if is_customer:
                    # Upsert as customer
                    customer = session.query(Customer).filter_by(xero_contact_id=contact_id).first()
                    if not customer:
                        customer = Customer(
                            code=contact.get("ContactNumber") or f"XERO-{contact_id[:8]}",
                            name=name,
