...
SCOPES = os.getenv("XERO_SCOPES", "offline_access accounting.contacts accounting.transactions").split()
...
def exchange_code_for_tokens(code: str) -> Dict[str, Any]:
    ...
    expires_at = datetime.now(timezone.utc) + timedelta(seconds=int(token_data.get("expires_in", 1800)))
...
def ensure_fresh_token(session: Optional[Session] = None) -> XeroToken:
    ...
    new_expires_at = datetime.now(timezone.utc) + timedelta(seconds=int(token_data.get("expires_in", 1800)))
