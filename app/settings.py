...
class APISettings(BaseSettings):
    ...
    # CORS settings
    cors_origins: List[str] = Field(default=["http://localhost:3000", "http://127.0.0.1:8050"])
...
class ShopifySettings(BaseSettings):
    ...
    store: AnyHttpUrl = Field(
        default="https://yourstore.myshopify.com",
    ...
    location_id: Optional[str] = Field(
        default=None,
        description="Shopify location ID (for multi-location setups)"
    )
    webhook_secret: Optional[str] = Field(
        default=None,
        description="Shopify webhook secret for HMAC verification"
    )
