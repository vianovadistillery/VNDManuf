...
def register_batch_processing_callbacks(
            ...
            if response.status_code == 200:
                ...
                for formula in formulas:
                    ...
                    if code and code not in formula_codes:
                        formula_codes[code] = {"code": code, "name": name}
    ...
    def load_formula_versions(formula_code):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                default_value = (
                    str(versions_sorted[0].get("id", "")) if versions_sorted else None
                )
    ...
    @app.callback(
        [Output("batch-formula-lines", "data"), Output("batch-multiplier", "value")],
        [
    ...
    def calculate_batch_multiplier(
        formula_id, target_production, target_unit, formula_code
    ):
        ...
        try:
            ...
            if target_unit == "L":
                if product_density:
                    ...
                    print(
                        f"Converted {target_production} L to {target_kg} kg using density {product_density}"
                    )
                else:
                    print(
                        f"Warning: Cannot convert {target_production} L to kg - product density not available"
                    )
            elif target_unit == "ea":
                # ea is typically for count-based items (non-mass based),
                # but for now we'll use the target as-is if provided
                print("Warning: 'ea' unit conversion not fully implemented")
                ...
                try:
                    ...
                    if soh_response.status_code == 200:
                        ...
                        for result in soh_results:
                            ...
                            soh_data[product_id] = (
                                round(float(soh_kg), 3) if soh_kg else 0.0
                            )
                except Exception as e:
    ...
    def create_batch(
        ...
        if not formula_id or not target_production or not target_unit:
            return (
                True,
                "Please fill in recipe, revision, and target production",
                "Validation Error",
            )
        ...
        try:
            ...
            if formula_response.status_code != 200:
                return (
                    True,
                    f"Error loading formula: {formula_response.status_code}",
                    "Error",
                )
            ...
            batch_response.json()  # Parse response
    ...
    @app.callback(
        ...
        [Input("batch-history-refresh", "n_clicks"), Input("main-tabs", "active_tab")],
        [State("batch-history-year", "value"), State("batch-history-status", "value")],
        prevent_initial_call=False,
    )
    ...
    def record_batch_actuals(
        n_clicks, selected_rows, batch_data, actual_kg, actual_litres, notes
    ):
