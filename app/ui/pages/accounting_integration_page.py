...
from app.services.xero_integration import push_supplier, push_customer, push_product_as_item, post_batch_journal, pull_contacts
...
class AccountingIntegrationPage:
    ...
    def layout(self):
        ...
        with get_session() as session:
            logs = session.query(XeroSyncLog).order_by(XeroSyncLog.ts.desc()).limit(100).all()
            log_data = [
                {
            ...
            dbc.CardBody([
                html.P([
                    ...
                    html.Span(
                        ...
                        className="badge bg-success" if is_connected else "badge bg-danger",
                    ),
                ]) if is_connected else html.P([
                    html.Strong("Status: "),
                ...
                ]) if token and token.tenant_id else None,
                html.Hr(),
                dbc.ButtonGroup([
                    dbc.Button(
                        "Connect to Xero",
        ...
        logs_card = dbc.Card([
            ...
            dbc.CardBody([
                dash.dash_table.DataTable(
                    ...
                    sort_action="native",
                    sort_mode="multi",
                ) if not log_df.empty else html.P("No sync logs yet"),
            ]),
        ])
    ...
    def register_callbacks(self, app):
        ...
        def handle_xero_actions(
            pull_n, push_sup_n, push_cust_n, push_prod_n, post_journal_n, refresh_n,
            supplier_id, customer_id, product_id, batch_no, amount,
        ):
