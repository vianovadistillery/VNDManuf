...
class CostingPage:
    ...
    def get_layout():
        return dbc.Container([
            ...
            dbc.Row([
                dbc.Col([
                    html.H2("Cost of Goods Inspection", className="mb-4")
                ])
            ]),
                    ...
                    dbc.Card([
                        dbc.CardBody([
                            dbc.Row([
                                ...
                                dbc.Col([
                                    dbc.Label("As Of Date (Point-in-Time):"),
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            html.Div(id="cogs-summary", children=[
                                html.P("Select a product and click 'Inspect COGS' to view cost breakdown.", className="text-muted")
                            ])
                        ])
                    ])
                ])
                        ...
                        dbc.CardBody([
                            dash_table.DataTable(
                                ...
                                columns=[
                                    ...
                                    {"name": "SKU", "id": "sku"},
                                    {"name": "Name", "id": "name"},
                                    {"name": "Qty/Unit", "id": "qty_per_unit", "type": "numeric", "format": {"specifier": ".6f"}},
                                    {"name": "Unit Cost", "id": "unit_cost", "type": "numeric", "format": {"specifier": ".2f"}},
                                    {"name": "Extended Cost", "id": "extended_cost", "type": "numeric", "format": {"specifier": ".2f"}},
                                    {"name": "Cost Source", "id": "cost_source"},
                                    {"name": "Has Estimate", "id": "has_estimate"},
                                    {"name": "Estimate Reason", "id": "estimate_reason"},
                                ],
                                data=[],
                                sort_action="native",
                                ...
                                style_data_conditional=[
                                    {
                                        'if': {'filter_query': '{has_estimate} = True'},
                                        'backgroundColor': '#fff4e6',
                                        'color': 'black',
                                    },
                                    {
                                        'if': {'filter_query': '{cost_source} = estimated'},
    ...
    def register_callbacks(app, api_base_url: str = "http://127.0.0.1:8000/api/v1"):
        ...
        def update_product_options(search_value):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    options = [
                        {"label": f"{p.get('sku', 'N/A')} - {p.get('name', 'Unnamed')}", "value": p.get("id")}
        ...
        def inspect_cogs(n_clicks, product_id, as_of_date, include_estimates):
            ...
            if not product_id or not n_clicks:
                return (
                    [html.P("Select a product and click 'Inspect COGS' to view cost breakdown.", className="text-muted")],
                    [html.Pre("No cost breakdown available.", className="text-muted", style={"fontFamily": "monospace"})],
                    []
                )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Summary
                    summary_items = [
                        html.H5(f"{cogs_breakdown.get('name', 'Unknown')} ({cogs_breakdown.get('sku', 'N/A')})"),
                        dbc.Row([
                            dbc.Col([
                                html.Strong("Unit Cost: "),
                                html.Span(f"${cogs_breakdown.get('unit_cost', 0):.2f}", className="text-success")
                            ]),
                            dbc.Col([
                                html.Strong("Cost Source: "),
                                html.Span(cogs_breakdown.get('cost_source', 'unknown'), className="badge bg-info")
                            ]),
                            dbc.Col([
                                html.Strong("Has Estimate: "),
                                html.Span(
                                    "Yes" if cogs_breakdown.get('has_estimate') else "No",
                                    className="badge bg-warning" if cogs_breakdown.get('has_estimate') else "badge bg-success"
                                )
                            ]),
                        ]),
                    ]
                    ...
                    return (
                        ...
                        [html.Pre(f"Error loading COGS: {error_msg}", className="text-danger", style={"fontFamily": "monospace"})],
                        []
                    )
            except requests.exceptions.ConnectionError:
                return (
                    [dbc.Alert("API not available. Please ensure the API server is running.", color="warning")],
                    [html.Pre("API not available.", className="text-muted", style={"fontFamily": "monospace"})],
                    []
                )
            except Exception as e:
                ...
                return (
                    [dbc.Alert(error_msg, color="danger")],
                    [html.Pre(error_msg, className="text-danger", style={"fontFamily": "monospace"})],
                    []
                )
...
def flatten_cogs_tree(node: Dict[str, Any], level: int = 0, parent_qty: float = 1.0) -> List[Dict[str, Any]]:
    ...
    rows.append({
        ...
        "estimate_reason": estimate_reason[:100] if estimate_reason else ""  # Truncate long reasons
    })
