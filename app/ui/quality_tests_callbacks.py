...
def register_quality_tests_callbacks(app, make_api_request):
    ...
    def open_quality_test_modal(add_clicks, edit_clicks, selected_rows, data):
        ...
        if button_id == "add-quality-test-btn":
            # Add mode - clear form
            return [True, "Add Quality Test", "", "", None, "", None, None, None, "", True, ""]
            ...
            return (
                ...
                test.get("is_active") == "Yes" if isinstance(test.get("is_active"), str) else test.get("is_active", True),
    ...
    def save_quality_test(n_clicks, code, name, test_type, unit, min_value, target_value, max_value, description, is_active, test_id):
        ...
        if not code or not name:
            return [True, "Error", True, "Error", "Code and Name are required", dash.no_update]
        ...
        try:
            if test_id:  # Update existing
                response = make_api_request("PUT", f"/quality-tests/{test_id}", test_data)
                if "error" in response:
                    return [True, "Error", True, "Error", f"Failed to update quality test: {response['error']}", dash.no_update]
                message = f"Quality test '{code}' updated successfully"
            else:  # Create new
                response = make_api_request("POST", "/quality-tests/", test_data)
                if "error" in response:
                    return [True, "Error", True, "Error", f"Failed to create quality test: {response['error']}", dash.no_update]
            ...
            return [True, "Error", True, "Error", f"Failed to save quality test: {str(e)}", dash.no_update]
    ...
    def delete_quality_test(n_clicks, selected_rows, data):
        ...
        try:
            ...
            if "error" in response:
                return dash.no_update, True, "Error", f"Failed to delete quality test: {response['error']}"
            ...
            return tests, True, "Success", f"Quality test '{test_code}' deleted successfully"
        
        except Exception as e:
            return dash.no_update, True, "Error", f"Failed to delete quality test: {str(e)}"
