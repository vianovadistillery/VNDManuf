...
def register_raw_materials_callbacks(app, make_api_request):
    ...
    @app.callback(
        ...
        [Input("rm-search-btn", "n_clicks"),
         Input("rm-refresh-btn", "n_clicks")],
        State("rm-search-filter", "value"),
        State("rm-status-filter", "value"),
        prevent_initial_call=False
    )
    def load_raw_materials(n_clicks_search, n_clicks_refresh, search_text, status_filter):
        ...
        try:
            ...
            if ctx.triggered and ctx.triggered[0]["prop_id"] == "rm-refresh-btn.n_clicks":
    ...
    @app.callback(
        ...
        [Input("rm-add-btn", "n_clicks"),
         Input("rm-edit-btn", "n_clicks")],
    ...
    @app.callback(
        Output("rm-modal", "is_open", allow_duplicate=True),
        [Input("rm-modal-cancel", "n_clicks"),
    ...
    def save_raw_material(n_clicks, desc1, desc2, search_key, search_ext, notes, xero_account, sg, purchase_cost, purchase_unit, usage_cost, usage_unit, deal_cost, sup_unit, sup_qty, soh, opening_soh, soh_value, restock_level, hazard, condition, msds_flag, active_flag, supplier_id, title, selected_rows, current_data):
        """Save or update raw material."""
        if not n_clicks:
            raise PreventUpdate
        
        if not desc1:
            print(f"Validation error: Description 1 is required")
            return no_update
        
        try:
            # Determine if this is edit mode
            is_edit = title == "Edit Raw Material" and selected_rows and len(selected_rows) > 0
            rm_id = None
            ...
            if is_edit:
                ...
                existing_codes = [int(rm.get("code", 0)) for rm in all_raw_materials if rm.get("code")]
            ...
            if supplier_id:
                ...
                if not new_rm_id:
                    ...
                    if all_rms and not "error" in all_rms:
                ...
                if new_rm_id:
                    ...
                    if is_edit:
                        ...
                        existing_suppliers = make_api_request("GET", f"/raw-materials/{new_rm_id}/suppliers")
                        if not "error" in existing_suppliers and isinstance(existing_suppliers, list):
                            for old_supplier in existing_suppliers:
                                old_sup_id = old_supplier.get("id")
                                if old_sup_id and old_sup_id != supplier_id:
                                    # Remove old supplier
                                    make_api_request("DELETE", f"/raw-materials/{new_rm_id}/suppliers/{old_sup_id}")
                    
                    # Add new supplier
                    supplier_payload = {"supplier_id": supplier_id}
                    supplier_response = make_api_request("POST", f"/raw-materials/{new_rm_id}/suppliers", supplier_payload)
                    if "error" in supplier_response:
                        print(f"Failed to add supplier: {supplier_response.get('error')}")
    ...
    def load_suppliers(is_open, selected_rows, data, title):
        ...
        try:
            ...
            options = [{"label": s.get("name", ""), "value": s.get("id")} for s in response]
            ...
            if title == "Edit Raw Material" and selected_rows and data:
                ...
                if rm_id:
                    suppliers_response = make_api_request("GET", f"/raw-materials/{rm_id}/suppliers")
                    if not "error" in suppliers_response and isinstance(suppliers_response, list) and len(suppliers_response) > 0:
