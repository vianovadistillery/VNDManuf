...
def register_units_callbacks(app, make_api_request):
    ...
    @app.callback(
        [Output("edit-unit-btn", "disabled"),
         Output("delete-unit-btn", "disabled")],
    ...
    def load_units_table(active_tab, refresh_clicks):
        """Load units table when settings tab is activated or refresh clicked."""
    ...
    def open_unit_modal(add_clicks, edit_clicks, selected_rows, data):
        ...
        if button_id == "add-unit-btn":
            ...
            return (
                ...
                unit.get("is_active") == "Yes" if isinstance(unit.get("is_active"), str) else unit.get("is_active", True),
    ...
    def save_unit(n_clicks, code, name, symbol, unit_type, description, is_active, unit_id):
        ...
        try:
            if unit_id:  # Update existing
                ...
                if "error" in response:
                    return [True, "Error", True, "Error", f"Failed to update unit: {response['error']}", dash.no_update]
                message = f"Unit '{code}' updated successfully"
            else:  # Create new
                response = make_api_request("POST", "/units/", unit_data)
                if "error" in response:
                    return [True, "Error", True, "Error", f"Failed to create unit: {response['error']}", dash.no_update]
            ...
            return [True, "Error", True, "Error", f"Failed to save unit: {str(e)}", dash.no_update]
    ...
    def delete_unit(n_clicks, selected_rows, data):
        ...
        try:
            ...
            if "error" in response:
                return dash.no_update, True, "Error", f"Failed to delete unit: {response['error']}"
