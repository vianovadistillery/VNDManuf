...
revision: str = "20251107_142500"
down_revision: Union[str, None] = None
branch_labels: Union[str, Sequence[str], None] = ("competitor_intel",)
...
def upgrade() -> None:
    ...
    if has_table(insp, "brands") and not has_index(insp, "brands", "ix_brands_name"):
        op.create_index("ix_brands_name", "brands", ["name"], unique=False)

    if not has_table(insp, "products"):
        op.create_table(
            "products",
            sa.Column("id", sa.String(length=36), primary_key=True, nullable=False),
    ...
    if not has_table(insp, "package_specs"):
        op.create_table(
            "package_specs",
            sa.Column("id", sa.String(length=36), primary_key=True, nullable=False),
            ...
            sa.UniqueConstraint(
                ...
                "container_ml",
                "can_form_factor",
                name="uq_package_specs_unique",
            ),
            sa.CheckConstraint(
                "type IN ('bottle','can')", name="ck_package_specs_type"
            ),
            sa.CheckConstraint(
    ...
    if not has_table(insp, "carton_specs"):
        op.create_table(
            "carton_specs",
            sa.Column("id", sa.String(length=36), primary_key=True, nullable=False),
    ...
    if not has_table(insp, "sku_cartons"):
        op.create_table(
            "sku_cartons",
            sa.Column("id", sa.String(length=36), primary_key=True, nullable=False),
            ...
            sa.UniqueConstraint(
                "sku_id", "carton_spec_id", name="uq_sku_cartons_parent"
            ),
        )

    if not has_table(insp, "companies"):
        op.create_table(
            "companies",
            sa.Column("id", sa.String(length=36), primary_key=True, nullable=False),
            sa.Column("name", sa.String(length=255), nullable=False),
            sa.Column(
                "type", sa.String(length=32), nullable=False, server_default="other"
            ),
    ...
    if not has_table(insp, "locations"):
        op.create_table(
            "locations",
            sa.Column("id", sa.String(length=36), primary_key=True, nullable=False),
    ...
    if not has_table(insp, "price_observations"):
        op.create_table(
            "price_observations",
            sa.Column("id", sa.String(length=36), primary_key=True, nullable=False),
            sa.Column("sku_id", sa.String(length=36), nullable=False),
            ...
            sa.Column(
                "gst_rate", sa.Numeric(6, 4), nullable=False, server_default="0.10"
            ),
            sa.Column(
    ...
    if has_table(insp, "price_observations"):
        index_specs = [
            (
            ...
            ("ix_price_observations_observation_dt", ["observation_dt"]),
            ("ix_price_observations_hash_key", ["hash_key"]),
        ]
        for index_name, columns in index_specs:
            if not has_index(insp, "price_observations", index_name):
                op.create_index(index_name, "price_observations", columns)

    if not has_table(insp, "attachments"):
        op.create_table(
            "attachments",
            sa.Column("id", sa.String(length=36), primary_key=True, nullable=False),
...
def downgrade() -> None:
    ...
    if has_table(insp, "price_observations"):
        ...
        for index_name in index_names:
            if has_index(insp, "price_observations", index_name):
                op.drop_index(index_name, table_name="price_observations")
    ...
    if has_table(insp, "brands"):
        if has_index(insp, "brands", "ix_brands_name"):
