...
def upgrade() -> None:
    ...
    sa.ForeignKeyConstraint(['formula_id'], ['formulas.id'], name=op.f('fk_finished_goods__formula_id__formulas')),
    ...
    sa.ForeignKeyConstraint(['fg_id'], ['finished_goods.id'], name=op.f('fk_finished_goods_inventory__fg_id__finished_goods'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('fg_id', name=op.f('pk_finished_goods_inventory'))
    )
    op.create_table('batch_lines',
    sa.Column('id', sa.String(length=36), nullable=False),
    ...
    sa.ForeignKeyConstraint(['batch_id'], ['batches.id'], name=op.f('fk_batch_lines__batch_id__batches'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['material_id'], ['products.id'], name=op.f('fk_batch_lines__material_id__products')),
    ...
    sa.ForeignKeyConstraint(['source_batch_id'], ['batches.id'], name=op.f('fk_inventory_movements__source_batch_id__batches')),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_inventory_movements'))
    )
    op.create_index('ix_movements_batch', 'inventory_movements', ['source_batch_id'], unique=False)
    op.create_index('ix_movements_date', 'inventory_movements', ['date'], unique=False)
    op.create_index('ix_movements_item', 'inventory_movements', ['item_type', 'item_id'], unique=False)
    op.add_column('batches', sa.Column('batch_status', sa.String(length=20), nullable=True))
