...
def upgrade() -> None:
    ...
    op.create_table('quality_test_definitions',
    ...
    op.create_index(op.f('ix_quality_test_definitions_code'), 'quality_test_definitions', ['code'], unique=True)
    ...
    op.add_column('qc_results', sa.Column('test_definition_id', sa.String(length=36), nullable=True))
    op.create_foreign_key(op.f('fk_qc_results__test_definition_id__quality_test_definitions'), 'qc_results', 'quality_test_definitions', ['test_definition_id'], ['id'])
    ...
    op.add_column('work_order_lines', sa.Column('actual_quantity_kg', sa.Numeric(precision=12, scale=3), nullable=True))
...
def downgrade() -> None:
    ...
    op.drop_constraint(op.f('fk_qc_results__test_definition_id__quality_test_definitions'), 'qc_results', type_='foreignkey')
    op.drop_column('qc_results', 'test_definition_id')
    ...
    op.drop_index(op.f('ix_quality_test_definitions_code'), table_name='quality_test_definitions')
