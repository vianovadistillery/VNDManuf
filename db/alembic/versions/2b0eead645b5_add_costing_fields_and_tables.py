...
def upgrade() -> None:
    ...
    with op.batch_alter_table('products', schema=None) as batch_op:
        if 'is_tracked' not in products_columns:
            batch_op.add_column(sa.Column('is_tracked', sa.Boolean(), nullable=True, server_default='1'))
        if 'sellable' not in products_columns:
            batch_op.add_column(sa.Column('sellable', sa.Boolean(), nullable=True, server_default='0'))
        if 'standard_cost' not in products_columns:
            batch_op.add_column(sa.Column('standard_cost', sa.Numeric(10, 2), nullable=True))
        if 'estimated_cost' not in products_columns:
            batch_op.add_column(sa.Column('estimated_cost', sa.Numeric(10, 2), nullable=True))
        ...
        if 'estimated_by' not in products_columns:
            batch_op.add_column(sa.Column('estimated_by', sa.String(100), nullable=True))
    ...
    with op.batch_alter_table('inventory_lots', schema=None) as batch_op:
        if 'original_unit_cost' not in lots_columns:
            batch_op.add_column(sa.Column('original_unit_cost', sa.Numeric(10, 2), nullable=True))
        if 'current_unit_cost' not in lots_columns:
            batch_op.add_column(sa.Column('current_unit_cost', sa.Numeric(10, 2), nullable=True))
    ...
    with op.batch_alter_table('inventory_txns', schema=None) as batch_op:
        ...
        if 'extended_cost' not in txns_columns:
            batch_op.add_column(sa.Column('extended_cost', sa.Numeric(12, 2), nullable=True))
        if 'estimate_flag' not in txns_columns:
            batch_op.add_column(sa.Column('estimate_flag', sa.Boolean(), nullable=True, server_default='0'))
    ...
    with op.batch_alter_table('assemblies', schema=None) as batch_op:
        if 'is_energy_or_overhead' not in assemblies_columns:
            batch_op.add_column(sa.Column('is_energy_or_overhead', sa.Boolean(), nullable=True, server_default='0'))
        ...
        if 'yield_factor' not in assemblies_columns:
            batch_op.add_column(sa.Column('yield_factor', sa.Numeric(6, 4), nullable=True, server_default='1.0'))
    ...
    if 'revaluations' not in tables:
        op.create_table(
            ...
            sa.Column('revalued_at', sa.DateTime(), nullable=False, server_default=sa.func.current_timestamp()),
            sa.Column('propagated_to_assemblies', sa.Boolean(), nullable=True, server_default='0'),
            sa.ForeignKeyConstraint(['item_id'], ['products.id'], name='fk_reval_item'),
            sa.ForeignKeyConstraint(['lot_id'], ['inventory_lots.id'], name='fk_reval_lot'),
        )
    ...
    if 'assembly_cost_dependencies' not in tables:
        op.create_table(
            ...
            sa.Column('dependency_ts', sa.DateTime(), nullable=False, server_default=sa.func.current_timestamp()),
            sa.ForeignKeyConstraint(['consumed_lot_id'], ['inventory_lots.id'], name='fk_dep_consumed_lot'),
            sa.ForeignKeyConstraint(['produced_lot_id'], ['inventory_lots.id'], name='fk_dep_produced_lot'),
            sa.ForeignKeyConstraint(['consumed_txn_id'], ['inventory_txns.id'], name='fk_dep_consumed_txn'),
            sa.ForeignKeyConstraint(['produced_txn_id'], ['inventory_txns.id'], name='fk_dep_produced_txn'),
        )
        op.create_index('ix_dep_consumed', 'assembly_cost_dependencies', ['consumed_lot_id'])
        op.create_index('ix_dep_produced', 'assembly_cost_dependencies', ['produced_lot_id'])
        op.create_index('ix_dep_consumed_txn', 'assembly_cost_dependencies', ['consumed_txn_id'])
        op.create_index('ix_dep_produced_txn', 'assembly_cost_dependencies', ['produced_txn_id'])
