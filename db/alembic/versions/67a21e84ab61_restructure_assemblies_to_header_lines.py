...
def upgrade() -> None:
    ...
    if "assembly_lines" not in tables:
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["assembly_id"],
                ["assemblies.id"],
                name=op.f("fk_assembly_lines__assembly_id__assemblies"),
            ),
            sa.ForeignKeyConstraint(
                ["component_product_id"],
                ["products.id"],
                name=op.f("fk_assembly_lines__component_product_id__products"),
            ),
            sa.PrimaryKeyConstraint("id", name=op.f("pk_assembly_lines")),
            sa.UniqueConstraint(
                "assembly_id", "sequence", name="uq_assembly_line_sequence"
            ),
        )
        op.create_index(
            "ix_assembly_line_assembly", "assembly_lines", ["assembly_id"], unique=False
        )
    ...
    with op.batch_alter_table("assemblies") as batch_op:
        if "assembly_code" not in assemblies_columns:
            batch_op.add_column(
                sa.Column("assembly_code", sa.String(length=50), nullable=True)
            )
        if "assembly_name" not in assemblies_columns:
            batch_op.add_column(
                sa.Column("assembly_name", sa.String(length=200), nullable=True)
            )
        if "notes" in assemblies_columns:
            # Check current type and only alter if needed
            current_col = next(
                col
                for col in inspector.get_columns("assemblies")
                if col["name"] == "notes"
            )
            if (
                hasattr(current_col["type"], "length")
                and current_col["type"].length == 255
            ):
                batch_op.alter_column(
                    "notes", type_=sa.Text(), existing_type=sa.VARCHAR(length=255)
                )
    ...
    if "ix_assemblies_primary" not in indexes:
        op.create_index(
            "ix_assemblies_primary",
            "assemblies",
            ["parent_product_id", "is_primary", "is_active"],
            unique=False,
        )
    ...
    with op.batch_alter_table("assemblies") as batch_op:
        batch_op.add_column(
            sa.Column(
                ...
                server_default="MAKE_FROM_CHILDREN",
                nullable=False,
            )
        )
        batch_op.add_column(
            sa.Column("child_product_id", sa.VARCHAR(length=36), nullable=True)
        )
        batch_op.add_column(
        ...
        batch_op.add_column(
            sa.Column("loss_factor", sa.NUMERIC(precision=6, scale=4), nullable=True)
        )
        batch_op.alter_column(
            "notes", type_=sa.VARCHAR(length=255), existing_type=sa.Text()
        )
