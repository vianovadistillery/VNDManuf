...
def upgrade() -> None:
    ...
    try:
        op.add_column('excise_rates', sa.Column('is_active', sa.Boolean(), nullable=True, server_default='1'))
    ...
    try:
        op.create_unique_constraint('uq_excise_rate_date', 'excise_rates', ['date_active_from'])
    except Exception:
    ...
    op.execute("UPDATE products SET is_purchase = COALESCE(is_purchase, 0) WHERE is_purchase IS NULL")
    op.execute("UPDATE products SET is_sell = COALESCE(is_sell, 0) WHERE is_sell IS NULL")
    op.execute("UPDATE products SET is_assemble = COALESCE(is_assemble, 0) WHERE is_assemble IS NULL")
    
    # Add indexes for capability columns if they don't exist
    try:
        op.create_index('ix_products_is_purchase', 'products', ['is_purchase'], unique=False)
    except Exception:
        pass
    try:
        ...
        pass
    try:
        op.create_index('ix_products_is_assemble', 'products', ['is_assemble'], unique=False)
    except Exception:
        pass
