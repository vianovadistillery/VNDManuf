...
def upgrade() -> None:
    ...
    with op.batch_alter_table('products', schema=None) as batch_op:
        ...
        if 'raw_material_group_id' not in products_columns:
            batch_op.add_column(sa.Column('raw_material_group_id', sa.String(length=36), nullable=True))
        
        # Raw Material specific fields
        if 'raw_material_code' not in products_columns:
            batch_op.add_column(sa.Column('raw_material_code', sa.Integer(), nullable=True))
        if 'raw_material_search_key' not in products_columns:
            batch_op.add_column(sa.Column('raw_material_search_key', sa.String(length=5), nullable=True))
        if 'raw_material_search_ext' not in products_columns:
            batch_op.add_column(sa.Column('raw_material_search_ext', sa.String(length=8), nullable=True))
        if 'specific_gravity' not in products_columns:
            batch_op.add_column(sa.Column('specific_gravity', sa.Numeric(precision=10, scale=6), nullable=True))
        if 'vol_solid' not in products_columns:
            batch_op.add_column(sa.Column('vol_solid', sa.Numeric(precision=10, scale=6), nullable=True))
        if 'solid_sg' not in products_columns:
            batch_op.add_column(sa.Column('solid_sg', sa.Numeric(precision=10, scale=6), nullable=True))
        if 'wt_solid' not in products_columns:
            batch_op.add_column(sa.Column('wt_solid', sa.Numeric(precision=10, scale=6), nullable=True))
        if 'usage_unit' not in products_columns:
            batch_op.add_column(sa.Column('usage_unit', sa.String(length=2), nullable=True))
        if 'usage_cost' not in products_columns:
            batch_op.add_column(sa.Column('usage_cost', sa.Numeric(precision=10, scale=2), nullable=True))
        if 'restock_level' not in products_columns:
            batch_op.add_column(sa.Column('restock_level', sa.Numeric(precision=12, scale=3), nullable=True))
        if 'used_ytd' not in products_columns:
            batch_op.add_column(sa.Column('used_ytd', sa.Numeric(precision=12, scale=3), nullable=True))
        ...
        if 'condition' not in products_columns:
            batch_op.add_column(sa.Column('condition', sa.String(length=1), nullable=True))
        if 'msds_flag' not in products_columns:
            batch_op.add_column(sa.Column('msds_flag', sa.String(length=1), nullable=True))
        ...
        if 'last_movement_date' not in products_columns:
            batch_op.add_column(sa.Column('last_movement_date', sa.String(length=8), nullable=True))
        if 'last_purchase_date' not in products_columns:
            batch_op.add_column(sa.Column('last_purchase_date', sa.String(length=8), nullable=True))
        if 'ean13_raw' not in products_columns:
            batch_op.add_column(sa.Column('ean13_raw', sa.Numeric(precision=18, scale=4), nullable=True))
        if 'xero_account' not in products_columns:
            batch_op.add_column(sa.Column('xero_account', sa.String(length=50), nullable=True))
        
        # Finished Good specific fields
        if 'formula_id' not in products_columns:
            batch_op.add_column(sa.Column('formula_id', sa.String(length=36), nullable=True))
        if 'formula_revision' not in products_columns:
            batch_op.add_column(sa.Column('formula_revision', sa.Integer(), nullable=True))
    ...
    if 'product_migration_map' not in tables:
        ...
        op.create_index('ix_migration_map_legacy', 'product_migration_map', ['legacy_table', 'legacy_id'])
    ...
    if 'raw_materials' in tables:
        ...
        for rm in raw_materials:
            ...
            description = (rm.desc1 or "") + " " + (rm.desc2 or "") if rm.desc2 else rm.desc1
            ...
            """)
            existing = connection.execute(existing_query, {"sku": sku, "code": rm.code}).fetchone()
            ...
            if rm.soh and rm.soh > 0:
                ...
                unit_cost = float(rm.soh_value) / float(rm.soh) if rm.soh_value and rm.soh else None
                connection.execute(lot_query, {
                    "lot_id": lot_id,
                    "product_id": product_id,
                    "lot_code": lot_code,
                    "qty": rm.soh,
    ...
    if 'formula_lines' in tables:
        ...
        formula_columns = [col['name'] for col in inspector.get_columns('formula_lines')]
    ...
    try:
        op.create_foreign_key(
            ...
            'products', 'raw_material_groups',
            ['raw_material_group_id'], ['id']
        )
    except Exception:
