"""Migrate foreign keys from customers/suppliers to unified contacts

Revision ID: migrate_to_unified_contacts
Revises: fix_products_pk_sqlite
Create Date: 2025-11-04T10:55:10.794040
...
def upgrade() -> None:
    ...
    try:
        op.add_column(
            "customer_prices", sa.Column("contact_id", sa.String(36), nullable=True)
        )
        op.create_foreign_key(
            "fk_customer_prices_contact",
            "customer_prices",
            "contacts",
            ["contact_id"],
            ["id"],
    ...
    try:
        ...
        op.create_foreign_key(
            "fk_invoices_contact", "invoices", "contacts", ["contact_id"], ["id"]
        )
    except Exception as e:
    ...
    try:
        op.add_column(
            "sales_orders", sa.Column("contact_id", sa.String(36), nullable=True)
        )
        op.create_foreign_key(
            "fk_sales_orders_contact",
            "sales_orders",
            "contacts",
            ["contact_id"],
            ["id"],
    ...
    try:
        op.add_column(
            "purchase_orders", sa.Column("contact_id", sa.String(36), nullable=True)
        )
        op.create_foreign_key(
            "fk_purchase_orders_contact",
            "purchase_orders",
            "contacts",
            ["contact_id"],
            ["id"],
    ...
    try:
        ...
        op.create_foreign_key(
            "fk_raw_material_suppliers_contact",
            "raw_material_suppliers",
            "contacts",
            ["contact_id"],
            ["id"],
    ...
    print(
        "[INFO] Added contact_id columns. Old customer_id/supplier_id columns remain for backward compatibility."
    )
...
def downgrade() -> None:
    ...
    try:
        op.drop_constraint(
            "fk_purchase_orders_contact", "purchase_orders", type_="foreignkey"
        )
    ...
    try:
        op.drop_constraint(
            "fk_sales_orders_contact", "sales_orders", type_="foreignkey"
        )
    ...
    try:
        op.drop_constraint(
            "fk_customer_prices_contact", "customer_prices", type_="foreignkey"
        )
