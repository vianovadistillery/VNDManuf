...
def analyze_gaps():
    ...
    db_tables = {
        t for t in db_tables if not t.startswith("sqlite_") and t != "alembic_version"
    }
    ...
    if gaps["table_differences"]:
        ...
        for table_name, diff in gaps["table_differences"].items():
            ...
            if diff["columns_in_db_not_in_code"]:
                issues.append(
                    f"{len(diff['columns_in_db_not_in_code'])} missing columns"
                )
            ...
            if diff["column_differences"]:
                issues.append(
                    f"{len(diff['column_differences'])} type/nullable differences"
                )
