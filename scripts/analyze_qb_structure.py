...
def extract_includes(filepath: Path) -> Set[str]:
    ...
    with open(filepath, 'r', encoding='cp437', errors='ignore') as f:
        for line in f:
            ...
            match = re.search(r"\$INCLUDE[:\s]+['\"]([\w./]+)['\"]", line, re.IGNORECASE)
            if match:
...
def main():
    ...
    with open(output_dir / "qb_data_dictionary.csv", 'w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=['type', 'field', 'qbasic_type', 'source_file'])
        writer.writeheader()
    ...
    print(f"- qb_module_inventory.md")
    print(f"- qb_type_definitions.md")
    print(f"- qb_data_dictionary.csv")
    print(f"- qb_include_map.md")
