...
def complete_migration_cleanup():
    ...
    with engine.connect() as connection:
        ...
        keep_columns = [col for col in assemblies_columns if col not in existing_to_drop]
        
        # Get column definitions for columns to keep
        all_cols = inspector.get_columns("assemblies")
        keep_col_defs = {col["name"]: col for col in all_cols if col["name"] in keep_columns}
        ...
        try:
            ...
            cursor.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='assemblies'")
            ...
            print("Please run this via: alembic upgrade head (after fixing the migration issue)")
