...
def complete_cleanup():
    ...
    with engine.connect() as connection:
        ...
        if not existing_to_drop:
            ...
            missing_columns = [col for col in expected_columns if col not in assemblies_columns]
            if missing_columns:
        ...
        for idx in indexes:
            ...
            if hasattr(idx, 'column_names'):
                col_names = idx.column_names if isinstance(idx.column_names, list) else [idx.column_names]
            else:
                # For SQLite, column info might be in a different format
                idx_sql = cursor.execute(f"SELECT sql FROM sqlite_master WHERE type='index' AND name='{idx['name']}'").fetchone()
        ...
        for row in cursor.fetchall():
            if row[0] not in indexes_to_drop and row[0] != 'sqlite_autoindex_assemblies_1':
        ...
        for col in existing_to_drop:
            ...
            """)
            for row in cursor.fetchall():
                if row[0] not in indexes_to_drop and not row[0].startswith('sqlite_autoindex'):
                    indexes_to_drop.append(row[0])
        ...
        assemblies_columns_after = [col["name"] for col in inspector.get_columns("assemblies")]
        expected_columns = ["assembly_code", "assembly_name"]
        missing_columns = [col for col in expected_columns if col not in assemblies_columns_after]
