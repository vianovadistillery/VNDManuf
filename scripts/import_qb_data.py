...
def import_raw_material_groups(db) -> Dict[int, str]:
    ...
    for grp_data in groups:
        ...
        existing = (
            ...
            .filter(RawMaterialGroup.code == grp_data["code"])
            .first()
        )
...
def import_raw_materials(
    db, group_map: Dict[int, str], dry_run: bool = False
) -> List[Dict]:
    """
    ...
    for qb_rec in qb_records[:10]:  # Test with first 10 records
        ...
        try:
            ...
            anomalies.append(
                {"record": qb_rec.get("no", "unknown"), "error": str(e), "data": qb_rec}
            )
...
def main():
    ...
    parser.add_argument(
        "--allow-anomalies", action="store_true", help="Continue on anomalies"
    )
