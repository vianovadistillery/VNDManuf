...
class LegacyFileScanner:
    ...
    def _scan_bas_file(self, bas_file: Path) -> None:
        ...
        type_matches = re.finditer(type_pattern, content, re.MULTILINE | re.DOTALL | re.IGNORECASE)
        ...
        for match in field_matches:
            ...
            self._add_field_mapping(bas_file, field_name, 0, length, "STRING", "unknown", field_name)
    ...
    def _parse_type_definition(self, bas_file: Path, type_name: str, type_body: str) -> None:
        ...
        for line in lines:
            ...
            if field_match:
                ...
                self._add_field_mapping(
                    bas_file, f"{type_name}.{field_name}", 
                    offset, length, field_type_str, 
                    self._map_to_table(type_name), field_name
                )
                offset += length
    ...
    def _add_field_mapping(self, bas_file: Path, field_name: str, offset: int, 
                          length: int, field_type: str, table: str, column: str) -> None:
    ...
    def _create_placeholder_fields(self, spec: Dict[str, Any], record_length: int) -> None:
    ...
    def scan_data_files(self) -> None:
        ...
        for data_file in self.data_dir.glob("**/*"):
            if data_file.is_file() and not data_file.suffix.lower() in ['.bas', '.bi', '.txt']:
    ...
    def write_outputs(self) -> None:
        ...
        with mapping_csv.open("w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=[
                "legacy_file", "field", "offset", "length", "type", 
                "new_table", "new_column", "notes"
            ])
            writer.writeheader()
...
def main():
    ap = argparse.ArgumentParser(description="Scan legacy files and create mapping specs")
    ap.add_argument("--src", required=True, help="Source directory containing .BAS files")
    ap.add_argument("--data", required=True, help="Data directory containing legacy data files")
    ap.add_argument("--out", required=True, help="Output directory for mapping CSV and YAML specs")
