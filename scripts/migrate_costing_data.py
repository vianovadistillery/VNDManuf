...
from app.adapters.db.models import (
    Product, InventoryLot, InventoryTxn, Batch, BatchComponent,
    AssemblyCostDependency, InventoryTxn as Txn
)
...
def backfill_transaction_costing_fields(db: Session, dry_run: bool = True):
    ...
    for txn in txns:
        ...
        if txn.extended_cost is None and txn.unit_cost is not None and txn.quantity_kg is not None:
        ...
        if txn.cost_source is None:
            if txn.transaction_type == "RECEIPT":
                ...
                updates['cost_source'] = CostSource.FIFO_ACTUAL.value  # Assembly-produced items
            else:
                updates['cost_source'] = CostSource.UNKNOWN.value
...
def build_assembly_cost_dependencies(db: Session, dry_run: bool = True):
    ...
    for batch in batches:
        ...
        produced_lots = db.query(InventoryLot).filter(
            InventoryLot.product_id == batch.work_order.product_id,
            InventoryLot.received_at >= batch.completed_at if batch.completed_at else True
        ).order_by(InventoryLot.received_at.desc()).limit(1).all()
        ...
        produce_txns = db.query(InventoryTxn).filter(
        ...
        for component in batch.components:
            ...
            issue_txns = db.query(InventoryTxn).filter(
                InventoryTxn.lot_id == component.lot_id,
                InventoryTxn.transaction_type == "ISSUE",
                InventoryTxn.created_at <= (batch.completed_at or datetime.utcnow())
            ).order_by(InventoryTxn.created_at.desc()).limit(1).all()
            ...
            existing = db.query(AssemblyCostDependency).filter(
            ...
            if not dry_run:
                ...
                print(f"Would create dependency: {component.lot_id} -> {produced_lot.id}")
                created_count += 1
    
    if not dry_run:
        db.commit()
    
    print(f"{'Would create' if dry_run else 'Created'} {created_count} assembly cost dependencies")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Migrate costing data for existing records")
    parser.add_argument(
        "--execute",
        action="store_true",
