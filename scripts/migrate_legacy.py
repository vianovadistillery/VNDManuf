...
from app.adapters.db import create_tables, get_db
...
from app.adapters.legacy_io import (
    ...
    create_sample_pack_units_data,
    create_sample_products_data,
)
...
class LegacyMigrator:
    ...
    def __init__(
        self, db: Session, dry_run: bool = False, allow_anomalies: bool = False
    ):
        self.db = db
            ...
            try:
                ...
                existing = self.db.execute(
                    select(InventoryLot).where(
                        InventoryLot.lot_code == lot_data["lot_code"]
                    )
                ).scalar_one_or_none()
            ...
            try:
                ...
                work_order = self.db.execute(
                    select(WorkOrder).where(
                        WorkOrder.code == batch_data["work_order_id"]
                    )
                ).scalar_one_or_none()

                if not work_order:
                    raise ValueError(
                        f"Work order {batch_data['work_order_id']} not found"
                    )
    ...
    def migrate_invoices(self) -> None:
        ...
        for invoice_data in invoices_data:
            try:
                ...
                if not customer:
                    raise ValueError(
                        f"Customer {invoice_data['customer_id']} not found"
                    )
            ...
            for i, anomaly in enumerate(self.anomalies):
                ...
                with anomaly_file.open("w", newline="", encoding="utf-8") as f:
                    writer = csv.DictWriter(
                        f, fieldnames=["table", "error", "timestamp"]
                    )
                    writer.writeheader()
                    writer.writerow(
                        {
...
def main():
    ...
    ap.add_argument(
        "--dry-run", action="store_true", help="Don't write to database, just report"
    )
    ap.add_argument(
        "--allow-anomalies",
        action="store_true",
        help="Continue migration despite anomalies",
    )
    ap.add_argument(
        "--output", default="out", help="Output directory for audit reports"
    )
    ...
    try:
        migrator = LegacyMigrator(
            db, dry_run=args.dry_run, allow_anomalies=args.allow_anomalies
        )
