...
def get_column_info(column):
    ...
    try:
        col_info["nullable"] = (
            bool(column.nullable) if column.nullable is not None else None
        )
...
def get_table_info(model_class):
    ...
    for rel_name, rel_prop in inspector.relationships.items():
        rel_info = {
            ...
            "target_table": (
                rel_prop.entity.class_.__table__.name if rel_prop.entity else None
            ),
...
def find_all_models():
    ...
    try:
        from app.adapters.db import models, models_assemblies_shopify
        ...
        try:
            ...
            try:
                ...
                if table_name:
                    ...
                    print(
                        f"  - {model_class.__name__} -> {table_name} (partial - inspection error)"
                    )
                else:
