...
def test_batch_completion_with_wip_via_api():
    ...
    try:
        ...
        try:
            ...
            print(
                "   [SKIP] API server not running. Start with: python -m uvicorn app.api.main:app --reload"
            )
            return True
...
def test_batch_completion_with_wip_service(db: Session):
    ...
    try:
        ...
        wip_products = (
            db.query(Product)
            .filter(
                Product.product_type == "WIP", Product.sku.like(f"WIP-{finished.sku}%")
            )
            .all()
        )
...
def test_multi_stage_batch_workflow(db: Session):
    ...
    try:
        ...
        finished_batch = batch_service.finish_batch(
            batch_id=batch.id, qty_fg_kg=Decimal("95.0"), create_wip=True
        )
        db.flush()
        ...
        print(
            f"   [OK] Stage 2: Produced {result['produced']['quantity_kg']} kg FINISHED from WIP"
        )
        ...
        print("   [OK] Multi-stage workflow completed successfully")
...
def test_batch_with_existing_wip(db: Session):
    ...
    try:
        ...
        wip_lots = (
            db.query(InventoryLot)
            .filter(InventoryLot.product_id == existing_wip.id)
            .all()
        )
        ...
        print(
            f"   [OK] Inventory lot created: {wip_lots[0].lot_code}, qty: {wip_lots[0].quantity_kg} kg"
        )
...
def test_batch_component_consumption(db: Session):
    ...
    try:
        ...
        db.refresh(lot1)
        db.refresh(lot2)
        print("   [OK] Components consumed via unified products:")
