...
def test_invalid_product_type_api():
    ...
    try:
        ...
        if response.status_code == 422:
            ...
            print(
                f"   [FAIL] Expected 422, got {response.status_code}: {response.text}"
            )
            return False
    except Exception as e:
        print(f"   [ERROR] {e}")
...
def test_missing_assembly_definition(db: Session):
    ...
    try:
        ...
        try:
            ...
            error_msg = str(e)
            if (
                "No assembly definitions" in error_msg
...
def test_missing_product_assembly_api():
    ...
    try:
        ...
        fake_id = "00000000-0000-0000-0000-000000000000"
        assemble_data = {"parent_product_id": fake_id, "qty": 10.0, "reason": "TEST"}

        response = requests.post(
            f"{API_BASE_URL}/assemblies/assemble", json=assemble_data, timeout=5
        )
