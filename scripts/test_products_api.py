...
try:
    ...
    print("API server is not running. Please start it with: uvicorn app.api.main:app --reload")
...
try:
    response = requests.get(f"{API_BASE_URL}/products/", params={"product_type": "RAW"}, timeout=5)
    print(f"Status Code: {response.status_code}")
    if response.status_code == 200:
        data = response.json()
