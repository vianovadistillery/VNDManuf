...
from app.adapters.db.models import Product, Formula, FormulaLine, InventoryLot, WorkOrder, Batch
...
def test_formula_lines_with_migrated_products(db: Session):
    ...
    try:
        ...
        for line in formula_lines:
            ...
            assert line.product.product_type in ["RAW", "WIP", "FINISHED"], \
        ...
        print(f"   [OK] All formula_lines reference valid products")
        print(f"   [OK] Backward compatibility property works")
        return True
...
def test_assembly_with_migrated_products(db: Session):
    ...
    try:
        ...
        raw1_lots = db.query(InventoryLot).filter(InventoryLot.product_id == raw1.id).all()
        raw2_lots = db.query(InventoryLot).filter(InventoryLot.product_id == raw2.id).all()
        ...
        print(f"   [OK] Migrated RAW products can be used for assembly:")
    ...
    try:
        ...
        for line in lines:
            if line.product and line.product.usage_cost:
                ...
                print(f"         - {line.product.sku}: {line.quantity_kg} kg @ {line.product.usage_cost} = {line_cost}")
...
def test_batch_operations_with_migrated_products(db: Session):
    ...
    try:
        ...
        if work_orders:
            ...
            print(f"   [OK] Migrated products can be used in batches")
        else:
            print("   [INFO] No work orders found, but products are ready for batch use")
