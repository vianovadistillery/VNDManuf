...
def validate_migration():
    ...
    try:
        if 'formula_lines' in inspector.get_table_names():
            ...
            if orphaned > 0:
                issues.append(f"{orphaned} formula_lines reference non-existent products")
    ...
    try:
        if 'inventory_movements' in inspector.get_table_names():
            ...
            if 'item_type' in columns or 'item_id' in columns:
                warnings.append("inventory_movements still has item_type/item_id columns (legacy)")
            ...
            if orphaned > 0:
                issues.append(f"{orphaned} inventory_movements reference non-existent products")
    ...
    try:
        if 'inventory_lots' in inspector.get_table_names():
            ...
            if orphaned > 0:
                issues.append(f"{orphaned} inventory_lots reference non-existent products")
