...
from app.adapters.db.qb_models import RawMaterial, FinishedGood
...
def validate_migration(db_url: str):
    ...
    try:
        ...
        if 'formula_lines' in inspector.get_table_names():
            ...
            if 'raw_material_id' in columns and 'product_id' in columns:
                ...
                null_product_id = session.execute(
                    text("SELECT COUNT(*) FROM formula_lines WHERE product_id IS NULL AND raw_material_id IS NOT NULL")
                ).scalar()
                
                if null_product_id > 0:
                    issues.append(f"Found {null_product_id} formula_lines with raw_material_id but no product_id")
                else:
        ...
        if 'inventory_movements' in inspector.get_table_names():
            columns = [col['name'] for col in inspector.get_columns('inventory_movements')]
            
            if 'item_type' in columns and 'product_id' in columns:
                # Check if any movements still use old structure
                null_product_id = session.execute(
                    text("SELECT COUNT(*) FROM inventory_movements WHERE product_id IS NULL AND item_id IS NOT NULL")
                ).scalar()
                
                if null_product_id > 0:
                    issues.append(f"Found {null_product_id} inventory_movements with item_id but no product_id")
                else:
        ...
        if 'raw_materials' in inspector.get_table_names():
            ...
            if raw_product_count < raw_material_count:
                ...
                successes.append(f"Raw materials appear migrated: {raw_product_count} RAW products found")
        ...
        if 'formula_lines' in inspector.get_table_names():
            ...
            if orphaned_lines > 0:
                issues.append(f"Found {orphaned_lines} formula_lines with invalid product_id references")
            else:
        ...
        if orphaned_lots > 0:
            issues.append(f"Found {orphaned_lots} inventory_lots with invalid product_id references")
        else:
