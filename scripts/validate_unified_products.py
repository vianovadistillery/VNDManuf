...
def validate_formula_lines(db):
    ...
    try:
        ...
        if raw_material_count > 0:
            print(
                f"WARNING: {raw_material_count} formula_lines still have raw_material_id (legacy column)"
            )
        else:
...
def validate_inventory_movements(db):
    ...
    if invalid_count > 0:
        print(
            f"ERROR: {invalid_count} inventory_movements reference non-existent products"
        )
        return False
    ...
    try:
        ...
        if legacy_count > 0:
            print(
                f"WARNING: {legacy_count} inventory_movements still have item_type/item_id (legacy columns)"
            )
        else:
...
def validate_inventory_lots(db):
    ...
    null_product_query = text(
        "SELECT COUNT(*) FROM inventory_lots WHERE product_id IS NULL"
    )
    null_count = db.execute(null_product_query).scalar()
