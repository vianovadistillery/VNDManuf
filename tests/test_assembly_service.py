...
def test_assemble_multiple_children(db_session: Session):
    ...
    # Create products
    parent = Product(
        sku="PARENT-003", name="Parent", product_type="FINISHED", base_unit="KG"
    )
    child1 = Product(
        sku="CHILD-003A", name="Child A", product_type="RAW", base_unit="KG"
    )
    child2 = Product(
        sku="CHILD-003B", name="Child B", product_type="RAW", base_unit="KG"
    )
    ...
    # Check quantities
    child1_consumed = next(
        c for c in result["consumed"] if c["child_product_id"] == child1.id
    )
    child2_consumed = next(
        c for c in result["consumed"] if c["child_product_id"] == child2.id
    )
...
def test_assemble_insufficient_stock(db_session: Session):
    ...
    # Create products
    parent = Product(
        sku="PARENT-004", name="Parent", product_type="FINISHED", base_unit="KG"
    )
...
def test_multi_stage_assembly(db_session: Session):
    ...
    finished = Product(
        sku="FIN-MULTI", name="Finished", product_type="FINISHED", base_unit="KG"
    )
