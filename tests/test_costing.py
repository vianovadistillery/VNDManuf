...
from app.domain.rules import CostSource
...
class TestDataIntegrity:
    ...
    def test_ledger_balances_match_lots(self, db_session):
        ...
        ledger_sum = (
            db_session.query(func.sum(InventoryTxn.quantity_kg))
            .filter(InventoryTxn.lot_id == lot.id)
            .scalar()
        )
...
class TestRevaluationPropagation:
    ...
    def test_revaluation_propagates(self, db_session):
        ...
        assembly_service = AssemblyService(db_session)
        result = assembly_service.assemble(
            parent_product_id=wip.id, parent_qty=Decimal("50.0"), reason="Test assembly"
        )
        db_session.commit()
        ...
        deps_before = (
            db_session.query(AssemblyCostDependency)
            .filter(
        ...
        revals = (
            db_session.query(Revaluation).filter(Revaluation.lot_id == wip_lot_id).all()
        )
