...
from app.adapters.db.models import FormulaLine, InventoryLot, InventoryMovement, Product
...
def test_migration_mapping_table(db_session: Session):
    ...
    found = db_session.query(Product).filter(Product.raw_material_code == 9999).first()
...
def test_inventory_movement_migration(db_session: Session):
    ...
    finished = Product(
        sku="FG-MOV", name="Finished", product_type="FINISHED", base_unit="KG"
    )
...
def test_product_type_defaults(db_session: Session):
    ...
    product = Product(sku="DEFAULT-TEST", name="Default Product", base_unit="KG")
    db_session.add(product)
    db_session.flush()

    # Verify default
    assert (
        product.product_type == "RAW" or product.product_type is None
    )  # Depending on default handling


def test_backward_compatibility_formula_line(db_session: Session):
    """Test backward compatibility property on FormulaLine."""
    raw = Product(
        sku="BC-TEST", name="Backward Compat Test", product_type="RAW", base_unit="KG"
    )
    db_session.add(raw)
    db_session.flush()
    ...
    finished = Product(
        sku="BC-FG", name="Finished", product_type="FINISHED", base_unit="KG"
    )
...
def test_product_type_filtering_query(db_session: Session):
    ...
    finished_count = (
        db_session.query(Product).filter(Product.product_type == "FINISHED").count()
    )
...
def test_unified_inventory_across_types(db_session: Session):
    ...
    finished = Product(
        sku="INV-FG", name="Finished", product_type="FINISHED", base_unit="KG"
    )
