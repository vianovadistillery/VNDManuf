...
from app.adapters.db.models import (
    Product, WorkOrder, Formula, Batch, InventoryLot, InventoryTxn
)
...
def test_finish_batch_create_wip(db_session: Session):
    ...
    wip_products = db_session.query(Product).filter(
        Product.product_type == "WIP",
        Product.sku.like(f"WIP-{product.sku}%")
    ).all()
    ...
    wip_lots = db_session.query(InventoryLot).filter(
    ...
    txn = db_session.query(InventoryTxn).filter(
...
def test_finish_batch_to_finished(db_session: Session):
    ...
    finished_batch = service.finish_batch(
        batch.id,
        qty_fg_kg=Decimal("73.0")
    )
    db_session.commit()
    
    # Verify finished goods lot was created
    fg_lots = db_session.query(InventoryLot).filter(
